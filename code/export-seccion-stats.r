...
#############################################################################
## cherrypick code from script below (from Chiapas local district drawing) ##
#############################################################################

# read municipios
#tmp <- paste(md, edo, sep = "") # archivo con mapas rojano
tmp <- paste("../../../fed/shp/disfed2017/", edo, sep = "") # archivo con mapas 2017
mu.map <- readOGR(dsn = tmp, layer = 'MUNICIPIO')
# polygon area in km2
...
# c. from ine's 2017 distrito maps
tmp <- paste("../../../fed/shp/disfed2017/", edo, sep = "") # archivo con mapas 2017
...
tmp <- merge(x = tmp, y = se.map[,c("seccion","disfed2006","disfed2017","disloc2017","disloc2012")], by = "seccion", all.x = TRUE, all.y = FALSE)
...
##########################
# read shapefiles 1er escenario 2017
...
par(mar=c(0,0,2,0)) ## SETS B L U R MARGIN SIZES
#par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
plot(p84(ed.map$cps), col = "white", axes = TRUE, main = "Chiapas (mapa local 2017)")#, bg = "lightblue")
plot(p84(ed.map$oax), col = "white", add = TRUE, lty = 3)
plot(p84(ed.map$ver), col = "white", add = TRUE, lty = 3)
plot(p84(ed.map$tab), col = "white", add = TRUE, lty = 3)
plot(p84(ed.map$cam), col = "white", add = TRUE, lty = 3)
...
plot(se.map[se.map$disloc2017==dn,], add = TRUE, border = "darkgray", col = alpha(portray[se.map$disloc2017==dn], .5)) # color nwin
# add casillas
...
#
#plot(ed.map$nay, add = TRUE, lty = 1, col = rgb(1,1,1, alpha = .5)) # blurs colors inside state
plot(se.map[se.map$disloc2017==dn,], add = TRUE, border = "darkgray", col = alpha(portray2[se.map$disloc2017==dn], .5)) # color nwin
# add casillas
points(cas.map, pch = 20, col = "white" , cex = .3)
